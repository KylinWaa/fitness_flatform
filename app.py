...
def checkin_page():
    ...
    if today_checkin and today_checkin.status == 'active' and today_checkin.start_time:
        start_timestamp = today_checkin.start_time.replace(tzinfo=BEIJING_TZ).timestamp()
...
def api_checkin_start():
    ...
    c = CheckIn(user_id=current_user.id, check_date=today, start_time=beijing_now(), status='active')
    db.session.add(c)
    db.session.commit()

    broadcast_bark('🏋️ 开始打卡', f'{current_user.username} 在 {today} 开始了健身打卡')
...
def api_public_checkin_start(username):
    ...
    c = CheckIn(user_id=user.id, check_date=today, start_time=beijing_now(), status='active')
    db.session.add(c)
    db.session.commit()

    broadcast_bark('🏋️ 不打卡是给', f'{user.username} 在 {today} 开始了健身打卡')
...
def api_schedule_update():
    ...
    db.session.add(s)
    current_user.schedule_modified_at = beijing_now()
    db.session.commit()
